export async function sha256(input: string): Promise<string> {
  ...
  if (subtle) {
    try {
      ...
      // fall through to JS implementation (e.g. insecure context)
    }
  }
  return sha256Fallback(data);
}

// Pure-JS SHA-256 used when WebCrypto SubtleCrypto is unavailable
// (e.g. when the site is served over plain http:// on a .onion address,
// which is not a "secure context" in some browsers).
